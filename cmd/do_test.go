...
func TestDoCmdRunE(t *testing.T) {
	t.Run("Run without app container", func(t *testing.T) {
		ctx := context.Background()

		testCmd := &cobra.Command{Use: "test-do"}
...
func TestDoCmdIntegration(t *testing.T) {
	t.Run("Command is added to root", func(t *testing.T) {
		...
		if !found {
			t.Error("Do command should be added to root command")
		}
	})
