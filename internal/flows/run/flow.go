...
// Package run implements the multi-step run workflow.
package run
...
func (f *Factory) runDoFlow(ctx context.Context, flowCtx *executor.Context) error {
	...
	flowCtx.SendRunningWithDetails("I'm running the task", fmt.Sprintf("goal=%q", initialGoal))
	...
	flowCtx.SendCompletedWithDetails("I've finished the task", fmt.Sprintf("goal=%q", initialGoal))
