...
func (r *ToolRunner) RunTool(ctx context.Context, execCtx *executor.Context, tool, mode string, params json.RawMessage, resolvedProfile *profile.ResolvedProfile, systemPrompt string) (*agentturn.ToolResult, error) {
	r.toolCallCount++
	toolLabel := fmt.Sprintf("Tool:%s.%s#%d", strings.ToLower(tool), strings.ToLower(mode), r.toolCallCount)

	// Parse params to map for message generation
	var paramsMap map[string]any
	if len(params) > 0 {
		if err := json.Unmarshal(params, &paramsMap); err != nil {
			// ignore error, paramsMap will be empty
			_ = err
		}
	}

	td := agentturn.GetToolDescription(strings.ToLower(tool), strings.ToLower(mode))

	var toolCtx *executor.Context
	detailsBase := fmt.Sprintf("tool=%s mode=%s call=%d", strings.ToLower(tool), strings.ToLower(mode), r.toolCallCount)
	if execCtx != nil {
		toolCtx = execCtx.Child(toolLabel)
		runningMsg := fmt.Sprintf("I'm running tool %s.%s", strings.ToLower(tool), strings.ToLower(mode))
		details := detailsBase
		if td != nil {
			detailAction := strings.TrimSpace(td.GetRunningMessage(paramsMap))
			if detailAction != "" {
				details = fmt.Sprintf("%s action=%q", detailsBase, detailAction)
			}
		}
		toolCtx.SendRunningWithDetails(runningMsg, details)
	}

	result, err := r.dispatchTool(ctx, toolCtx, tool, mode, params, resolvedProfile, systemPrompt)

	if err != nil {
		if toolCtx != nil {
			toolCtx.SendFailedWithDetails(err, "I couldn't run the tool", detailsBase)
		}
		return nil, err
	}

	if toolCtx != nil {
		completedMsg := fmt.Sprintf("I've finished tool %s.%s", strings.ToLower(tool), strings.ToLower(mode))
		details := detailsBase
		if td != nil {
			detailAction := strings.TrimSpace(td.GetCompletedMessage(result))
			if detailAction != "" {
				details = fmt.Sprintf("%s result=%q", detailsBase, detailAction)
			}
			tdDetails := td.GetDetailsMessage(paramsMap, result)
			if tdDetails != "" {
...
func (r *ToolRunner) dispatchTool(ctx context.Context, toolCtx *executor.Context, tool, mode string, params json.RawMessage, resolvedProfile *profile.ResolvedProfile, systemPrompt string) (*agentturn.ToolResult, error) {
	switch strings.ToLower(tool) {
	case "workspace":
		return r.runWorkspaceTool(ctx, toolCtx, mode, params)
	case "search":
		return r.runSearchTool(ctx, toolCtx, mode, params)
	case "git":
		return r.runGitTool(ctx, toolCtx, mode, params)
	case "summarize":
		return r.runSummarizeTool(ctx, toolCtx, mode, params, resolvedProfile, systemPrompt)
	case "plan":
		return r.runPlanTool(mode, params)
	case "memory":
		return r.runMemoryTool(mode, params)
	case "command":
		return r.runCommandTool(ctx, toolCtx, mode, params)
	case "patch":
		return r.runPatchTool(ctx, toolCtx, mode, params)
	default:
...
func (r *ToolRunner) runWorkspaceTool(ctx context.Context, _ *executor.Context, mode string, params json.RawMessage) (*agentturn.ToolResult, error) {
...
func (r *ToolRunner) handleWorkspaceList(ctx context.Context, params json.RawMessage) (*agentturn.ToolResult, error) {
	...
	// Convert to []interface{} for tool details rendering
	items := make([]interface{}, len(result))
	for i, item := range result {
		items[i] = item
	}

	return &agentturn.ToolResult{Tool: "workspace", Mode: modeList, Data: items}, nil
}

func (r *ToolRunner) handleWorkspaceRead(params json.RawMessage) (*agentturn.ToolResult, error) {
	...
	return &agentturn.ToolResult{Tool: "workspace", Mode: "read", Data: result}, nil
}

func (r *ToolRunner) handleWorkspaceWrite(params json.RawMessage) (*agentturn.ToolResult, error) {
	...
	return &agentturn.ToolResult{Tool: "workspace", Mode: "write", Data: result}, nil
}

func (r *ToolRunner) handleWorkspaceReplace(params json.RawMessage) (*agentturn.ToolResult, error) {
	...
	r.workspaceDirty = true
	return &agentturn.ToolResult{Tool: "workspace", Mode: "replace", Data: result}, nil
}

func (r *ToolRunner) handleWorkspaceDelete(params json.RawMessage) (*agentturn.ToolResult, error) {
	...
	r.workspaceDirty = true
	return &agentturn.ToolResult{Tool: "workspace", Mode: "delete", Data: result}, nil
}

func (r *ToolRunner) handleWorkspaceMkdir(params json.RawMessage) (*agentturn.ToolResult, error) {
	...
	r.workspaceDirty = true
	return &agentturn.ToolResult{Tool: "workspace", Mode: "mkdir", Data: result}, nil
}

func (r *ToolRunner) handleWorkspaceStat(params json.RawMessage) (*agentturn.ToolResult, error) {
	...
	return &agentturn.ToolResult{Tool: "workspace", Mode: "stat", Data: result}, nil
}

func (r *ToolRunner) handleWorkspaceExists(params json.RawMessage) (*agentturn.ToolResult, error) {
	var input workspaceStatParams
	if err := json.Unmarshal(params, &input); err != nil {
	...
	return &agentturn.ToolResult{Tool: "workspace", Mode: "exists", Data: result}, nil
}
...
func (r *ToolRunner) runSearchTool(ctx context.Context, execCtx *executor.Context, mode string, params json.RawMessage) (*agentturn.ToolResult, error) {
	...
	results := make([]interface{}, 0, len(output.Results))
	...
	return &agentturn.ToolResult{Tool: "search", Mode: "embeddings", Data: results}, nil
}
...
func (r *ToolRunner) runGitTool(_ context.Context, _ *executor.Context, mode string, params json.RawMessage) (*agentturn.ToolResult, error) {
	if r.gitService == nil {
		return nil, fmt.Errorf("git service is nil")
	}

	handlers := map[string]func(json.RawMessage) (*agentturn.ToolResult, error){
...
func (r *ToolRunner) handleGitStatus(_ json.RawMessage) (*agentturn.ToolResult, error) {
	status, err := r.gitService.Status()
	if err != nil {
		return nil, fmt.Errorf("git status failed: %w", err)
	}
	return &agentturn.ToolResult{Tool: "git", Mode: "status", Data: status}, nil
}

func (r *ToolRunner) handleGitDiff(params json.RawMessage) (*agentturn.ToolResult, error) {
...
func (r *ToolRunner) handleGitShow(params json.RawMessage) (*agentturn.ToolResult, error) {
	...
	return &agentturn.ToolResult{Tool: "git", Mode: "show", Data: show}, nil
}

func (r *ToolRunner) handleGitLog(params json.RawMessage) (*agentturn.ToolResult, error) {
	return handleGitWithParams(params, "log", func(input gitLogParams) (string, error) {
		return r.gitService.Log(input.Limit, input.Path)
	})
}

func (r *ToolRunner) handleGitBranch(_ json.RawMessage) (*agentturn.ToolResult, error) {
	...
	return &agentturn.ToolResult{Tool: "git", Mode: "branch", Data: items}, nil
}

func (r *ToolRunner) handleGitCurrentBranch(_ json.RawMessage) (*agentturn.ToolResult, error) {
	branch, err := r.gitService.CurrentBranch()
	if err != nil {
		return nil, fmt.Errorf("git current_branch failed: %w", err)
	}
	return &agentturn.ToolResult{Tool: "git", Mode: "current_branch", Data: branch}, nil
}

func (r *ToolRunner) handleGitStage(params json.RawMessage) (*agentturn.ToolResult, error) {
	...
	return &agentturn.ToolResult{Tool: "git", Mode: "stage", Data: output}, nil
}

func (r *ToolRunner) handleGitCommit(params json.RawMessage) (*agentturn.ToolResult, error) {
	...
	return &agentturn.ToolResult{Tool: "git", Mode: "commit", Data: output}, nil
}

func handleGitWithParams[T any](params json.RawMessage, mode string, action func(T) (string, error)) (*agentturn.ToolResult, error) {
	var input T
	if err := json.Unmarshal(params, &input); err != nil {
	...
	return &agentturn.ToolResult{Tool: "git", Mode: mode, Data: output}, nil
}
...
func (r *ToolRunner) runSummarizeTool(ctx context.Context, execCtx *executor.Context, mode string, params json.RawMessage, resolvedProfile *profile.ResolvedProfile, systemPrompt string) (*agentturn.ToolResult, error) {
	handlers := map[string]func(json.RawMessage) (*agentturn.ToolResult, error){
		"text": func(raw json.RawMessage) (*agentturn.ToolResult, error) {
			return r.summarizeText(ctx, execCtx, raw, resolvedProfile, systemPrompt)
		},
		"file": func(raw json.RawMessage) (*agentturn.ToolResult, error) {
			return r.summarizeFile(ctx, execCtx, raw, resolvedProfile, systemPrompt)
		},
		"diff": func(raw json.RawMessage) (*agentturn.ToolResult, error) {
...
func (r *ToolRunner) summarizeText(ctx context.Context, execCtx *executor.Context, params json.RawMessage, resolvedProfile *profile.ResolvedProfile, systemPrompt string) (*agentturn.ToolResult, error) {
	...
	return &agentturn.ToolResult{Tool: "summarize", Mode: "text", Data: summary}, nil
}

func (r *ToolRunner) summarizeFile(ctx context.Context, execCtx *executor.Context, params json.RawMessage, resolvedProfile *profile.ResolvedProfile, systemPrompt string) (*agentturn.ToolResult, error) {
	...
	return &agentturn.ToolResult{Tool: "summarize", Mode: "file", Data: summary}, nil
}

func (r *ToolRunner) summarizeDiff(ctx context.Context, execCtx *executor.Context, params json.RawMessage, resolvedProfile *profile.ResolvedProfile, systemPrompt string) (*agentturn.ToolResult, error) {
	...
	return &agentturn.ToolResult{Tool: "summarize", Mode: "diff", Data: summary}, nil
}
...
func (r *ToolRunner) runPlanTool(mode string, params json.RawMessage) (*agentturn.ToolResult, error) {
	switch strings.ToLower(mode) {
	case "add":
		var input struct {
			Text string `json:"text"`
		...
		return &agentturn.ToolResult{Tool: "plan", Mode: "add", Data: map[string]any{"id": item.ID, "text": item.Text, "completed": item.Completed}}, nil
	case "complete":
		var input struct {
		...
		for i, item := range r.planMemory.items {
			if item.ID == input.TaskID {
				...
				return &agentturn.ToolResult{Tool: "plan", Mode: "complete", Data: map[string]any{"id": updated.ID, "text": updated.Text, "completed": updated.Completed}}, nil
			}
		}
	...
	case modeList:
		...
		return &agentturn.ToolResult{Tool: "plan", Mode: modeList, Data: items}, nil
	default:
		return nil, fmt.Errorf("unsupported plan mode %q", mode)
	}
}

func (r *ToolRunner) runMemoryTool(mode string, params json.RawMessage) (*agentturn.ToolResult, error) {
	switch strings.ToLower(mode) {
	case "add":
		var input struct {
			Text string `json:"text"`
		...
		return &agentturn.ToolResult{Tool: "memory", Mode: "add", Data: map[string]any{"id": item.ID, "text": item.Text}}, nil
	case modeList:
		...
		return &agentturn.ToolResult{Tool: "memory", Mode: modeList, Data: items}, nil
	default:
...
func (r *ToolRunner) runCommandTool(ctx context.Context, _ *executor.Context, mode string, params json.RawMessage) (*agentturn.ToolResult, error) {
...
func (r *ToolRunner) runPatchTool(ctx context.Context, _ *executor.Context, mode string, params json.RawMessage) (*agentturn.ToolResult, error) {
	...
	return &agentturn.ToolResult{Tool: "patch", Mode: "apply", Data: map[string]string{"status": "applied"}}, nil
}
