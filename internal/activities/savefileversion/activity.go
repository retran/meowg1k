...
// Factory builds savefileversion activities.
type Factory struct {
...
// NewFactory creates a savefileversion activity factory.
func NewFactory(indexService ports.IndexService) (executor.ActivityFactory[*Input, *Output], error) {
	if indexService == nil {
		return nil, fmt.Errorf("savefileversion.NewFactory: indexService cannot be nil")
	}
...
func (f *Factory) NewActivity() executor.Activity[*Input, *Output] {
	return func(ctx context.Context, executorCtx *executor.Context, input *Input) (*Output, error) {
		executorCtx.SendRunningWithDetails("I'm saving the file version", fmt.Sprintf("file=%s", input.FilePath))
		...
		executorCtx.SendCompletedWithDetails(
			fmt.Sprintf("I've saved %s", input.FilePath),
