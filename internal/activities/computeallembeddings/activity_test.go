...
func (m *mockComputeBatchFactory) NewActivity() executor.Activity[*computeembeddingsbatch.Input, *computeembeddingsbatch.Output] {
	// The activity itself is mocked at the executor level, so this can be a no-op.
	return func(ctx context.Context, executorCtx *executor.Context, input *computeembeddingsbatch.Input) (*computeembeddingsbatch.Output, error) {
		return nil, nil
	}
}
...
func TestNewFactory(t *testing.T) {
	...
	t.Run("should fail with a nil factory", func(t *testing.T) {
		...
		expectedErr := "computeallembeddings.NewFactory: computeBatchFactory cannot be nil"
...
func TestActivity(t *testing.T) {
	...
	t.Run("should succeed with no batches", func(t *testing.T) {
		...
		emptyInput := &Input{
			StateName: "empty",
			PreparedBatches: &preparebatches.Output{
				ChunkResults: &chunkallfiles.Output{AllChunkTexts: []string{}},
				Batches:      []preparebatches.Batch{},
			},
		}
