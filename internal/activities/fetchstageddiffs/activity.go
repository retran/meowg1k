...
type Factory struct {
	fileDiffActivityFactory executor.ActivityFactory[*fetchstageddiff.Input, *git.FileChange]
}
...
func NewFactory(fileDiffActivityFactory executor.ActivityFactory[*fetchstageddiff.Input, *git.FileChange]) (*Factory, error) {
...
func (f *Factory) NewActivity() executor.Activity[*Input, *Output] {
	return func(ctx context.Context, executorCtx *executor.Context, input *Input) (*Output, error) {
		...
		executorCtx.SendRunningWithDetails(
			"I'm fetching staged diffs",
			fmt.Sprintf("files=%d", len(input.Files)),
		)
		...
		for _, file := range input.Files {
			...
			change, err := executor.ExecuteActivity[*fetchstageddiff.Input, *git.FileChange](
				ctx,
				exec,
				executorCtx,
				file,
				fetchFileDiff,
				&fetchstageddiff.Input{
					Filename: file,
				},
			)
			if err != nil {
		...
		executorCtx.SendCompletedWithDetails(
			"I've fetched the staged diffs",
			fmt.Sprintf("diffs=%d", len(changes)),
		)
