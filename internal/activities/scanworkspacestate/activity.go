...
func (f *Factory) NewActivity() executor.Activity[struct{}, *Output] {
	return func(ctx context.Context, executorCtx *executor.Context, _ struct{}) (*Output, error) {
		executorCtx.SendRunning("I'm scanning the workspace state")
		...
		executorCtx.SendCompleted(fmt.Sprintf("I scanned the workspace (%d head, %d staged, %d working)", len(headState), len(stageState), len(workdirState)))
		return result, nil
	}
}
