...
func (td *ToolDescription) getSearchRunningMessage(args map[string]any) string {
	if td.Mode == modeEmbeddings {
		if query, ok := args["query_text"].(string); ok && len(query) > 40 {
			return fmt.Sprintf("Searching: %s", query[:40])
		} else if query, ok := args["query_text"].(string); ok {
...
var toolRegistry = map[string]map[string]*ToolDescription{
	...
	toolSearch: {
		"embeddings": {
			...
			Parameters: schemaObject(map[string]any{
				"query_text": schemaString("Search query."),
			}, nil),
		},
	},
	toolGit: {
