...
// Factory builds prepareindex activities.
type Factory struct {
...
// NewFactory creates a prepareindex activity factory.
func NewFactory(indexService ports.IndexService) (executor.ActivityFactory[*Input, *Output], error) {
	if indexService == nil {
		return nil, fmt.Errorf("prepareindex.NewFactory: indexService cannot be nil")
	}
...
func (f *Factory) NewActivity() executor.Activity[*Input, *Output] {
	return func(ctx context.Context, executorCtx *executor.Context, input *Input) (*Output, error) {
		executorCtx.SendRunningWithDetails("I'm planning index updates", "scope=workspace")
		...
		executorCtx.SendCompletedWithDetails(
			"I've prepared the index plan",
			fmt.Sprintf("to_index=%d cached=%d", len(prepareResult.FilesToProcess), len(prepareResult.ExistingVersions)),
		)
