...
func (m *mockInvokeFactory) NewActivity() executor.Activity[*draftcontent.Input, *draftcontent.Output] {
	return func(_ context.Context, _ *executor.Context, _ *draftcontent.Input) (*draftcontent.Output, error) {
		if m.index >= len(m.outputs) {
			return &draftcontent.Output{Content: `{"type":"final","content":"done"}`}, nil
		}
