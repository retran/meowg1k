...
// Package agentturn implements a single agent step activity with tool use.
package agentturn
...
func handleResponse(ctx context.Context, execCtx *executor.Context, input *Input, stepName string, toolDefs []gateway.ToolDefinition, toolNameMap map[string]*ToolDescription, response *draftcontent.Output) (*Output, *ToolResult, error) {
...
func handleNonJSONResponse(execCtx *executor.Context, stepName string, toolDefs []gateway.ToolDefinition, content string, parseErr error) (*Output, *ToolResult, error) {
	...
	execCtx.SendCompleted(fmt.Sprintf("I've finished step %s", stepName))
