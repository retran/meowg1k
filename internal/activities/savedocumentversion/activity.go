...
func (f *Factory) NewActivity() executor.Activity[*Input, *Output] {
	return func(ctx context.Context, executorCtx *executor.Context, input *Input) (*Output, error) {
		executorCtx.SendRunning(fmt.Sprintf("Saving document: %s (%d chunks)", input.FilePath, len(input.Chunks)))
		...
		executorCtx.SendCompleted(fmt.Sprintf("Saved document: %s (%d chunks)", input.FilePath, len(input.Chunks)))
		return &Output{
