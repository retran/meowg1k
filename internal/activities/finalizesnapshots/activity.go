...
func (f *Factory) NewActivity() executor.Activity[*Input, struct{}] {
	return func(ctx context.Context, executorCtx *executor.Context, input *Input) (struct{}, error) {
		executorCtx.SendRunning(fmt.Sprintf("Finalizing snapshots (existing: %d, new: %d)", len(input.ExistingVersions), len(input.NewVersions)))
		...
		executorCtx.SendCompleted(fmt.Sprintf("Finalized snapshots (existing: %d, new: %d)", len(input.ExistingVersions), len(input.NewVersions)))
		return struct{}{}, nil
	}
}
