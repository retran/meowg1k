...
func (f *Factory) NewActivity() executor.Activity[*Input, struct{}] {
	return func(ctx context.Context, executorCtx *executor.Context, input *Input) (struct{}, error) {
		executorCtx.SendRunning("I'm finalizing index snapshots")
		...
		executorCtx.SendCompleted(fmt.Sprintf("I finalized snapshots (%d existing, %d new)", len(input.ExistingVersions), len(input.NewVersions)))
		return struct{}{}, nil
	}
}
