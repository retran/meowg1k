...
func TestActivity(t *testing.T) {
	t.Run("should succeed and send correct messages", func(t *testing.T) {
		...
		if feedbackMessages[0].Status != executor.StatusRunning || feedbackMessages[0].Message != fmt.Sprintf("Building index for %s", snapshotName) {
			t.Errorf("unexpected running message.\nExpected Status: %v, Message: %s\nGot Status:      %v, Message: %s",
				executor.StatusRunning, fmt.Sprintf("Building index for %s", snapshotName), feedbackMessages[0].Status, feedbackMessages[0].Message)
		}
